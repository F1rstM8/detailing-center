...
const AdminDashboard = () => {
  ...
  const stats = useMemo(() => {
    ...
    const newOrders = orders.filter((o) => o.status === ORDER_STATUS.NEW).length;
    const inProgressOrders = orders.filter((o) => o.status === ORDER_STATUS.IN_PROGRESS).length; 
    ...
    <main className="page-content admin-page">
      <div className="admin-container">
        <header className="admin-header">
          ...
          <p>{t("admin_subtitle", "Управление заявками и статусами автомобилей")}</p>
        </header>
        ...
        <div className="admin-content">
          ...
          <section className="admin-stats">
            <div className="stat-card">
              ...
              <div className="stat-value revenue">{stats.totalRevenue} {CURRENCY}</div>
            ...
            <div className="stat-card">
              ...
              <div className="stat-label">{t("stat_in_progress", "В работе")}</div>
            </div>
            <div className="stat-card">
              <div className="stat-icon">✅</div>
          ...
          {!orders || orders.length === 0 ? (
            <p className="no-orders">{t("admin_no_orders", "Новых заявок пока нет.")}</p>
          ) : (
              ...
              {orders.map((order) => (
                <div key={order.id} className="order-card">
                  <div className="order-header">
                    ...
                    <span className={`order-status ${ORDER_STATUS_CLASSES[order.status] || ""}`}>
                  ...
                  <div className="order-body">
                    <p>
                      <strong>{t("order_client", "Клиент:")}</strong> {order.customerName}
                    </p>
                    <p>
                      <strong>{t("order_phone", "Телефон:")}</strong> {order.customerPhone}
                    </p>
                    <div className="order-services">
                      <strong>{t("order_services_selected", "Выбранные услуги:")}</strong>
                      <ul>
                    ...
                    <p className="order-total">
                      <strong>{t("order_total_price", "Итого к оплате:")}</strong> {order.totalPrice} {CURRENCY}
                    </p>
                  </div>
